...
st.set_page_config(page_title="AI 주식 레이더 V6", page_icon="🚨", layout="wide", initial_sidebar_state="expanded")
...
.stApp {background: radial-gradient(circle at top left,#14213d 0,#080b12 38%,#05070b 100%); color:#f3f6ff;}
.block-container{padding-top:1rem;max-width:1280px}
.hero{padding:22px;border-radius:26px;background:linear-gradient(135deg,rgba(0,200,255,.22),rgba(120,0,255,.15));border:1px solid rgba(160,180,255,.22);margin-bottom:16px;box-shadow:0 14px 40px rgba(0,0,0,.25)}
.hero h1{font-size:2.15rem!important;margin-bottom:.2rem}
.card{border:1px solid rgba(180,190,220,.16);border-radius:22px;padding:17px;background:rgba(255,255,255,.055);margin-bottom:13px;box-shadow:0 10px 28px rgba(0,0,0,.18)}
.green{background:linear-gradient(135deg,rgba(0,210,120,.22),rgba(0,120,255,.09))}
.red{background:linear-gradient(135deg,rgba(255,55,80,.23),rgba(255,160,0,.08))}
.orange{background:linear-gradient(135deg,rgba(255,165,0,.20),rgba(255,255,255,.05))}
.blue{background:linear-gradient(135deg,rgba(0,140,255,.18),rgba(255,255,255,.05))}
.big{font-size:35px;font-weight:900;letter-spacing:-.5px}
...
.news{border:1px solid rgba(180,190,220,.16);border-radius:16px;padding:13px 15px;margin:9px 0;background:rgba(255,255,255,.052)}
...
def stars(v):
    return "★"*max(1,min(5,round(v/20))) + "☆"*(5-max(1,min(5,round(v/20))))
...
def ai_signal(news_score, info):
    ...
    s=max(0,min(100,int(s)))
    up=s; down=100-s
    if s>=75:return "강한 상승",up,down,"관심 높게. 단, 급하게 따라 사는 건 조심.",why
    if s>=60:return "상승 우세",up,down,"상승 쪽이 조금 더 유리함.",why
    if s<=30:return "강한 하락",up,down,"위험 신호가 강해서 관망 우선.",why
    if s<=42:return "하락 우세",up,down,"약세 쪽이라 새 매수는 신중.",why
    return "관망",up,down,"방향이 애매해서 추가 확인 필요.",why

def period_text():
    start=date.today()+timedelta(days=1)
    end=date.today()+timedelta(days=4)
    return f"{start.strftime('%m/%d')} ~ {end.strftime('%m/%d')} 기준"
...
def event_calendar():
    today=date.today()
    return [
        [today+timedelta(days=1),"미국 주요 경제 뉴스 확인","금리·환율 영향 가능"],
        [today+timedelta(days=2),"반도체 뉴스 집중 감시","삼성·하이닉스·엔비디아 영향"],
        [today+timedelta(days=3),"실적/가이던스 관련 뉴스 체크","예상보다 좋거나 나쁘면 변동 커짐"],
        [today+timedelta(days=5),"CPI/FOMC 관련 일정 확인","시장 전체 방향에 영향"],
        [today+timedelta(days=7),"SpaceX/우주 관련 뉴스 체크","관련 테마주 변동 가능"],
    ]
...
st.markdown("<div class='hero'><h1>🚨 AI 주식 레이더 V6</h1><div>딱딱한 표보다 오늘 봐야 할 흐름을 먼저 보여줌 · 다크모드 · 일정 캘린더 · 긴급 속보</div></div>",unsafe_allow_html=True)
st.caption(f"업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} · 예측기간: {period_text()}")

tabs=st.tabs(["🏠 브리핑","🚨 긴급 속보","📅 캘린더","📊 종목 카드","📰 뉴스","📈 차트","🎯 기록"])
...
with tabs[0]:
    if not df.empty:
        ...
        st.markdown(f"""
        <div class='card blue brief'>
        <div class='midtitle'>📌 오늘 한 줄 브리핑</div>
        오늘은 <b>{top['종목']}</b>가 가장 강하게 잡혀. 
        예측 기간은 <b>{period_text()}</b>이고, 상승 가능성은 <b>{top['상승확률']}%</b>야.<br>
        반대로 <b>{weak['종목']}</b>는 주의 신호가 상대적으로 높아.
        </div>
        ...
        st.dataframe(df[["종목","신호","상승확률","하락확률","뉴스점수","기간","메모","근거"]],use_container_width=True)
...
with tabs[1]:
    ...
    for stock,sc,lab,reason,x in all_news:
        if sc>=75 or sc<=30:
            hot_news.append((stock,sc,lab,reason,x))
    if hot_news:
        ...
        st.info("지금은 강한 속보 신호가 없어. 자동 새로고침 켜두면 계속 확인함.")

with tabs[2]:
    st.subheader("📅 이번에 볼 일정")
    for d,title,memo in event_calendar():
        st.markdown(f"<div class='card blue'><span class='badge'>{d.strftime('%m/%d')}</span><b>{title}</b><br><span class='tiny'>{memo}</span></div>",unsafe_allow_html=True)
