...
"soxl":"SOXL|SOXL","solx":"SOXL|SOXL","soxs":"SOXS|SOXS","sols":"SOXS|SOXS","tqqq":"TQQQ|TQQQ","sqqq":"SQQQ|SQQQ",
"레버리지":"KODEX 레버리지|122630.KS","kodex 레버리지":"KODEX 레버리지|122630.KS","곱버스":"KODEX 200선물인버스2X|252670.KS","인버스2x":"KODEX 200선물인버스2X|252670.KS",
"엔비":"엔비디아|NVDA","엔비디아":"엔비디아|NVDA","nvidia":"엔비디아|NVDA","nvda":"엔비디아|NVDA","테슬라":"테슬라|TSLA","tsla":"테슬라|TSLA",
"amd":"AMD|AMD","애플":"애플|AAPL","aapl":"애플|AAPL","마소":"마이크로소프트|MSFT","msft":"마이크로소프트|MSFT","구글":"알파벳|GOOGL","googl":"알파벳|GOOGL",
"현대차":"현대차|005380.KS","기아":"기아|000270.KS","카카오":"카카오|035720.KS","네이버":"NAVER|035420.KS","한화에어로":"한화에어로스페이스|012450.KS","두산에너빌리티":"두산에너빌리티|034020.KS"}
...
def kr_code(t): return t.replace(".KS","").replace(".KQ","")
...
def naver_kr_price(code):
    # 네이버 현재가 우선. 무료 공개 페이지라 증권사 호가와 완전 동일하진 않을 수 있음.
    try:
        url=f"https://finance.naver.com/item/main.naver?code={code}"
        r=requests.get(url,timeout=8,headers={"User-Agent":"Mozilla/5.0"})
        r.raise_for_status()
        html=r.text
        m=re.search(r'<p class="no_today">\s*<em class="no_up">\s*<span class="blind">([\d,]+)</span>|<p class="no_today">\s*<em class="no_down">\s*<span class="blind">([\d,]+)</span>|<p class="no_today">\s*<em class="no_unch">\s*<span class="blind">([\d,]+)</span>',html,re.S)
        if not m:
            m=re.search(r'<p class="no_today">.*?<span class="blind">([\d,]+)</span>',html,re.S)
            price_txt=m.group(1) if m else None
        else:
            price_txt=next(g for g in m.groups() if g)
        if not price_txt:return None
        last=float(price_txt.replace(",",""))
        # 전일 대비 % 가능하면 추출
        pm=re.search(r'<span class="blind">([\d\.]+)</span>\s*%</span>',html)
        pct=float(pm.group(1)) if pm else 0.0
        if "no_down" in html[:html.find("no_today")+3000] and pct: pct=-pct
        return last,pct,"Naver"
    except Exception:
        return None

@st.cache_data(ttl=30)
def yahoo_price_fast(t):
    try:
        y=yf.Ticker(t)
        fi=getattr(y,"fast_info",{}) or {}
        last=fi.get("last_price") or fi.get("regular_market_price") or None
        prev=fi.get("previous_close") or None
        if last is None:
        ...
        last=float(last); prev=float(prev) if prev else last
        return last,((last-prev)/prev*100 if prev else 0),"Yahoo"
    except Exception:
        return None
...
def price(t):
    ...
    if yp:
        last,chg,src=yp
        c=h["Close"].dropna() if h is not None and not h.empty else pd.Series([last])
        last_krw=last*fx_rate() if is_us(t) else last
        ma=lambda n: float(c.tail(n).mean()) if len(c) else last
        return {"hist":h if h is not None and not h.empty else pd.DataFrame({"Close":[last]}),"last":last,"last_krw":last_krw,"chg":chg,"ma5":ma(5),"ma20":ma(20),"ma60":ma(60),"hi20":float(c.tail(20).max()) if len(c) else last,"lo20":float(c.tail(20).min()) if len(c) else last,"source":src}
    return None
...
def ai_signal(news_score,info):
    ...
    sig="강한 상승" if s>=75 else "상승 우세" if s>=60 else "강한 하락" if s<=30 else "하락 우세" if s<=42 else "관망"
    return sig,s,100-s,good,bad
def period_text(): return f"{(date.today()+timedelta(days=1)).strftime('%m/%d')} ~ {(date.today()+timedelta(days=4)).strftime('%m/%d')} 기준"
def market_calendar():
    today=date.today()
...
def load_log():
    ...
    return pd.DataFrame(columns=["date","period","name","ticker","signal","up_pct","down_pct","price_krw"])
def save_log(rows):
...
for n,t in stocks.items():
    ...
    radar.append({"종목":n,"티커":t,"신호":sig,"상승확률":up,"하락확률":down,"현재가원":round(info["last_krw"]) if info else 0,"등락률":round(info["chg"],2) if info else 0,"좋은점":" · ".join(good+news_good),"주의점":" · ".join(bad+news_bad),"출처":info.get("source","") if info else ""})
